...
<body>
	...
	if((!empty($_POST["user"])) && (!empty($_POST["password"]))) {
		...
		if(preg_match("/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])[0-9A-Za-z!@#$%]{7,15}$/", $pass)) {
			$conexion = new mysqli("localhost", "root", "admin", "identificate2015");
			if($conexion) {
				...
				if($conexion->query($query)) {
					echo "<div class='alert alert-success text-center'><p>Login correcto</p></div>";
					echo "<p class='text-center'>Bienvenido, $user.</p>";
					echo "<a href='autenticacion.php'>Salida</a><br>";
				}
				else {
					echo "<div class='alert alert-danger text-center'><p>Login incorrecto</p></div>";
					echo "<p class='text-danger text-center'>Combinación usuario/password incorrecta.</p>";
			...
			echo "<p class='text-danger'>Password inválido, asegúrese que el formato de la contraseña corresponde con lo pedido anteriormente.</p><br>";
			echo "<a href='autenticacion.php'>Volver a la página anterior</a><br>";
		}
	}
	else {
