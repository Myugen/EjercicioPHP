...
<body>
	...
	$sql_creartabla1.= "ID INT AUTO_INCREMENT PRIMARY KEY NOT NULL,
	...
	if(!$conexion) {
		...
		if(!$resultado) {
			...
			// CREAR TABLA 1:
			$resultado=$conexion->query($sql_creartabla1);
			if (!$resultado) {
			...
			$resultado=$conexion->query($sql_insertarregistros1);
			if (!$resultado) {
			...
			// CREAR TABLA 2:
			$resultado=$conexion->query($sql_creartabla2);
			if (!$resultado) {
			...
			$resultado=$conexion->query($sql_insertarregistros2);
			if (!$resultado) {
			...
			// CREAR TABLA 3:
			$resultado=$conexion->query($sql_creartabla3);
			if (!$resultado) {
			...
			// INSERTAR REGISTROS EN TABLA 3:
			$resultado=$conexion->query($sql_insertarregistros3);
			if (!$resultado) {
				echo "ERROR: Imposible insertar registros iniciales en tabla $tabla3 (puede que ya existan esos registros o que no se tengan los permisos).<br>\n";
			}
			else {
				echo "Registros iniciales insertados satisfactoriamente en la Tabla $tabla3.<br>\n";
			}
